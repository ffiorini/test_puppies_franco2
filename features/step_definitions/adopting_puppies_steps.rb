...
When /^I select "([^"]*)" from the pay with dropdown$/ do |payment|
...
Then /^I should see "([^"]*)" as the name for (line_item \d+)$/ do |name, line_item|
  on(ShoppingCartPage).name_for_line_item(line_item).should include name
end

When /^I should see "([^"]*)" as the subtotal for (line_item \d+)$/ do |subtotal, line_item|
